...
| config_url        | string  |                     | URL or path (e.g. `/local/...`) to a YAML file containing card config. The file is fetched on render and merged on top of the inline config. See [External config file](#external-config-file). |
| cache_bust        | boolean | false               | When `true`, appends `?_cb=<timestamp>` to `config_url` to bypass the browser cache. Useful when the file is regenerated by an integration. |
...
Instead of defining `nodes` / `links` / `sections` inline, you can point the
card at a YAML file using `config_url`. The file is fetched after `setConfig`
and its contents are merged on top of the inline config.

This is useful when the chart topology is generated by another tool or
integration — e.g. a power aggregator that emits a card YAML whenever the
Home Assistant area registry changes. With `config_url`, the card picks up
the new topology automatically without a copy/paste step.
...
- Inline config is applied first (so the card renders immediately, even
  before the fetch completes).
- On successful fetch, the loaded YAML is merged **on top of** the inline
  config — keys present in both win from the file.
- `config_url` and `cache_bust` themselves are always preserved from the
  inline config, so subsequent reloads keep working.

A typical pattern is a minimal inline config that only specifies the loader
keys and lets the external file define everything else.

**Paths**

- Relative paths under `/local/...` resolve to `/config/www/...` and are
  served by HA with auth. The card uses `hass.fetchWithAuth` so these work
  without exposing the file publicly.
- Absolute `http(s)://` URLs also work if the target server is reachable
  from the browser and allows CORS.

**cache_bust**

Set `cache_bust: true` when the referenced file is regenerated frequently.
A `?_cb=<timestamp>` query parameter is appended so the browser fetches a
fresh copy every render. Leave it off (or `false`) for static files and
let the normal HTTP cache headers do their work.

**Errors**

If the fetch fails (bad path, 404, invalid YAML) the card shows the error
like any other runtime failure. Fix the file and the card recovers on the
next render.
